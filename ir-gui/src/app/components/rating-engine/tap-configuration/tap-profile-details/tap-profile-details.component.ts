...
@Component({
  selector: 'app-tap-profile-details',
  templateUrl: './tap-profile-details.component.html',
  standalone: false
})
export class TapProfileDetailsComponent implements OnInit {
  ...
  private fetchDetails(id: number): void {
    ...
    this.tapService.getTapProfileById(id).subscribe({
      next: (res: any) => {
      ...
      error: (err: any) => {
        ...
        this.commonService.toastError(
          err?.error?.errorMessage || 'Failed to load tap profile details'
        );
      }
    });
  }
